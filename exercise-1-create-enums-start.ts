/**
 * START: Follow the instructions below.
 */

import { LanguageServiceMode } from "typescript";

// The objects in the `countries` array use strings as the values of the
// `currency` properties.

// Complete this numeric enum so that it can be used instead of strings.
// Hint: Use the string values in the `currency` properties as the enum members.

enum Currency {
  Euro,
  Dollar,
}

// Update the type for the `currency` property in this interface to use
// the `Currency` enum.

interface Country {
  name: string;
  //currency: string; - the currency property must accept numeric type values in order to allow our enum type value to be passed to it
  currency: number;
}

// Replace the string values for the `currency` properties below with

//replaced our currency values with our enum members
const countries: Country[] = [
  {
    name: "France",
    currency: Currency.Euro,
  },
  {
    name: "United States of America",
    currency: Currency.Dollar,
  },
  {
    name: "Italy",
    currency: Currency.Euro,
  },
  {
    name: "New Zealand",
    currency: Currency.Dollar,
  },
];
console.log(countries);
// Create a string enum named `LanguageStatus`. Use it to replace the
// string values for the `status` properties in the objects below.

enum LanguageStatus {
  spanish = "spanish",
  english = "english",
  primary = "primary",
  secondary = "secondary",
}
const countryLanguages = [
  { language: LanguageStatus.spanish, status: LanguageStatus.primary },
  { language: LanguageStatus.english, status: LanguageStatus.secondary },
];

// ----
console.log(countryLanguages);
export {};
